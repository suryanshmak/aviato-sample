          ...
          <AnimatePresence>
            {props.open && (
              <motion.ul
                ...
                className={`bg-[#131313] absolute z-10 mt-1 sm:w-[20rem] bg-white shadow-lg max-h-80 rounded-md text-base ring-1 ring-[#4e4e4e] ring-opacity-5 sm:text-sm`}
                style={{ width: "calc(16vw + 10rem)" }}
                tabIndex={-1}
                role="listbox"
                aria-labelledby="listbox-label"
                aria-activedescendant="listbox-option-3"
              >
                <div className="p-1 flex items-center sticky top-0 z-10">
                  <BsSearch className="ml-2 w-5 h-5 text-[#5e5e5e]" />
                  <input
                    type="search"
                    name="search"
                    autoComplete="off"
                    className="bg-[#131313] border-0 outline-none focus:ring-0 w-full sm:text-sm"
                  ...
                  {COUNTRIES.filter((country) =>
                    ...
                    COUNTRIES.filter((country) =>
                      ...
                      return (
                        <li
                          key={`${props.id}-${index}`}
                          className="text-gray-900 cursor-pointer rounded-md p-2 hover:bg-[#343434] select-none relative flex items-center hover:bg-gray-50 transition"
                          id="listbox-option-0"
                          role="option"
                          onClick={() => {
